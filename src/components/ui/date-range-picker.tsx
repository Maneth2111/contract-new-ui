      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            ...
            <span className="min-w-0 flex-1 truncate">{tempRange?.from
              ? tempRange.to && tempRange.to.getTime() !== tempRange.from.getTime()
                ? `${format(tempRange.from, 'MM/dd/yyyy')} - ${format(tempRange.to, 'MM/dd/yyyy')}`
                : format(tempRange.from, 'MM/dd/yyyy')
              : placeholder}
            </span>
            {value?.from && (
              <X
                className="h-3.5 w-3.5 shrink-0 text-primary hover:text-primary/80 cursor-pointer"
                onClick={(e) => {
                  e.stopPropagation()
                  setTempRange(undefined)
                  onChange?.(undefined)
